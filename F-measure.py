...
def Evaluation(th_score, img, filepath, image_name):
    ...
    ## 创建全0背景下的detected
    filename = filepath + 'detected/out_ellipse.txt'
    if not is_txt_file(filename):
        raise Exception("%s 不是一个txt文件" % filename)
    pass
    label = []
    ...
    for r in range(sz_gt):
        for c in range(sz_test):
            if TestOverlap(img_gt[r], img_detect[c], th_score):
                overlap[r, c] = 255
            else:
                overlap[r, c] = 0
    # counter = 0
...
def TestOverlap(gt, test, th):
    fAND = cv.countNonZero(cv.add(gt, test))
    fOR = cv.absdiff(gt, test)
    fOR = cv.countNonZero(fOR)
    if fAND == 0:
        return False
    # fOR  = cv.countNonZero(cv.subtract(test, gt))
    # fAND = cv.countNonZero(gt + test)
    # fOR  = cv.countNonZero(gt - test)
    # fsim = fAND / fOR
    fsim = fOR / fAND
    return fsim >= th
...
if __name__ == "__main__":
    ...
    f_str = []
    file_dir = filepath + 'images/'
    f = open('f_measure_origin/f_measure_origin.txt',"w")
    ...
    for i in range(len(files)):
        ...
        # img_file = filepath + 'images/' + img_name + '.pgm'
        ...
        f_measure = Evaluation( TH_SCORE, img_gt, filepath, img_name.replace('.pgm','') )
        f_str.append(f_measure)
    with open('f_measure_origin/f_measure_origin.txt',"w") as f:    #设置文件对象
